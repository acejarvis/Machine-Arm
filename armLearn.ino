...
//version 0.9.8
// enable an robot arm to learn motion
...
int buttonLeft = 2, buttonRight = 4;
int threshLeft = 300, threshRight = 700;
...
void move_arm(){
  boolean armChanged = false;

  for (int i = 0; i < SERVOS; i++){
    value[i] = analogRead(i);              // read the mecharm value from analog port 0~3
    delay(2);
    currentAngle[i] = myservo[i].read();   // get existing angles

    if (value[i] > threshRight) {
      idle[i] = 0;      // reset idle because someone touch the mecharm
      armChanged = true;
      if (currentAngle[i] > MIN[i]) --currentAngle[i]; // adjust the angle according to mecharm direction
      if (i == SERVOS-1) currentAngle[i] = clawCloseangle;            // for claw, only open/close mode
    } else if (value[i] < threshLeft) {
      idle[i] = 0;
      armChanged = true;
...
void move_mem(){
  if (!autoPlayMode){
    if (learningMode){
      ...
      int buttonPressed = !digitalRead(buttonLeft);
      ...
      if (!digitalRead(buttonRight)) { // Right button pressed
        autoPlayMode = true;
      }
    }
  } else {
...
// auto mode for demo purpose, can be paused any time if mecharm is touched
void auto_mode(){
...
void autoPlay(){
  ...
  while(1){
    for (int i = 0; i < actionIndex; i++){
      ...
      // pause if mecharm touched, resume if touch again
      if (buttonPressed()) {
        learningMode = false;
        autoPlayMode = false;
        autoDemoMode = false;
...
// check if mecharm touched to stop the auto mode
boolean mecharmTouched(){
  int tmp = 500;

  for (int i = 0; i < SERVOS; i++){
    tmp = analogRead(i);
    delay(1);
    if (tmp > threshRight || tmp < threshLeft) return true;
  }
...
void adjustSpeed(){
  //if (learningMode) {
    for (int i = 0; i < SERVOS; i++){
      if (analogRead(i) > threshRight) demospeed++;
      if (analogRead(i) < threshLeft) demospeed--;
    }
...
boolean buttonPressed(){
  ...
  if(!buttonPreState && (!digitalRead(buttonRight) || !digitalRead(buttonLeft))) {
    buttonPreState = 1;
    return true;
  } else if (digitalRead(buttonRight) && digitalRead(buttonLeft)){
    buttonPreState = 0;
  }
  return false;
}
